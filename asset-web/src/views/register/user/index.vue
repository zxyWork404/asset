<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="注册是否审核通过" align="center" prop="resultType">
      ...
      <el-table-column label="注册人属性" align="center" prop="registerType" >
        <template scope="scope">
      ...
      <el-table-column label="创建时间" align="center" prop="registerCreateTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.registerCreateTime) }}</span>
        </template>
      </el-table-column>
...
<script>
import { list ,sendMessage,saveExamine,listExamine} from "@/api/register/user";
...
export default {
  ...
  methods: {
    ...
    handleExaminePass(row){
      ...
      this.examineParams.type = "1";
      this.examineParams.resultType = "1";
      this.$confirm('此操作注册审核, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
